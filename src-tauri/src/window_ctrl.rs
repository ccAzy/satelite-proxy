...
fn position_reachable<R: Runtime>(app: &AppHandle<R>, x: f64, y: f64, w: f64, h: f64) -> bool {
    ...
    // Enough exposed title bar for the user to drag the window back.
    const GRAB_W: f64 = 80.0;
    ...
    monitors.iter().any(|m| {
        ...
        let overlap_w = (x + w).min(right) - x.max(left);
        let overlap_h = (y + h).min(bottom) - y.max(top);
        overlap_w >= GRAB_W && overlap_h >= GRAB_H
    })
}
...
pub fn show_main<R: Runtime>(app: &AppHandle<R>) {
    ...
    if let Some(w) = app.get_webview_window("main") {
        ...
        let builder = WebviewWindowBuilder::new(app, "main", WebviewUrl::App("index.html".into()))
            ...
            .visible(true)
            .focused(true);
        // Without an explicit position the OS places each new window itself
        // (Windows cascades via CW_USEDEFAULT — every tray wake landed
        // somewhere else). Restore the remembered spot when it still lands on
        // a connected monitor; otherwise center like the config-created
        // cold-start window.
