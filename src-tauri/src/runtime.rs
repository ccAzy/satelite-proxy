...
use crate::config::{
    ...
    build_xray_sidecar_config, generate_api_secret, inspect_mihomo_config, inspect_singbox_config,
    inspect_xray_config, outbound_tag, write_active_config, write_active_yaml_config,
    write_custom_config, write_mihomo_sidecar_config, write_xray_sidecar_config, BuildOptions,
    SidecarPlan,
};
...
use crate::domain::{
    ChainHop, CustomConfigKind, Protocol, ProxyNode, RuntimeSource, SubscriptionSource,
};
                ...
                store
                    ...
                    .and_then(|s| match &s.source {
                        SubscriptionSource::Custom { content, kind } => {
                            Some((content.clone(), *kind))
                        }
                        _ => None,
                    })?;
            let core_kind = match cfg_kind {
                CustomConfigKind::Singbox => CoreKind::SingBox,
                CustomConfigKind::Mihomo => CoreKind::Mihomo,
                CustomConfigKind::Xray => CoreKind::Xray,
            };
            let has_tun = match cfg_kind {
                CustomConfigKind::Singbox => inspect_singbox_config(&content).has_tun,
                CustomConfigKind::Mihomo => inspect_mihomo_config(&content).has_tun,
                CustomConfigKind::Xray => inspect_xray_config(&content).has_tun,
            };
            (core_kind, has_tun)
        }
        ...
        if enabled {
            let port = if store.settings.runtime_source().is_custom() {
                self.custom_inbound_port.ok_or_else(|| {
                    AppError::Core(
                        "当前自定义配置没有可用的代理入站端口（mixed/http/socks），无法开启系统代理".into(),
                    )
                })?
            } else {
                store.settings.mixed_port
