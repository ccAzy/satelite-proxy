...
use crate::services::import::{
    import_from_custom, import_from_file, import_from_file_with_id, import_from_node,
...
pub async fn add_subscription_custom(
    ...
    let fresh_id = fresh_subscription_id(
        ...
        SubscriptionSource::Custom {
            content: body.clone(),
            // Only the content hash feeds uniqueness here; the placeholder
            // kind is irrelevant — the real kind is detected inside the
            // import and the id we pass is used verbatim.
            kind: crate::domain::CustomConfigKind::Singbox,
        },
    )?;
    let outcome = import_custom_blocking(name, body, Some(fresh_id)).await?;
    persist_import(&app, &state, outcome)
}
...
pub async fn update_subscription(
    ...
    let (outcome, replaced_id) = match kind.as_str() {
        ...
        _ => return Err("kind must be url, file, text, node, or custom".into()),
    };
...
fn persist_import_replacing(
    ...
    if let Some(body) = raw_body {
        if let Err(error) =
            crate::config::write_raw_subscription(&state.app_data_dir, &raw_id, &body)
        {
            crate::app_log::warn(
                "subscription",
                format!("{raw_id}: failed to persist raw config body: {error}"),
            );
        }
    }
    if node_set_changed {
