...
- `tray.rs` — 托盘菜单 + 图标状态刷新（9 种托盘图标 `TrayIconStyle`，`src-tauri/icons/tray/`；saturn=应用图标同款土星瓦片，off 灰/on 彩色）。**菜单文案双语（2026-09-21）**：`TRAY_LABELS_ZH/EN` 按 `settings.locale` 选取，`refresh_icon` 内 `refresh_labels` 全量重设（`update_settings` 无条件调 `refresh_icon`，故 UI 切语言托盘即时跟随）；新增菜单项时两处常量表都要加。
